...
public class AccountController {
	...
	@Autowired
	public AccountController(AccountService accountService, DtoConverter converter, UserService userService) {
	...
	public ResponseEntity<Object> createAccount(@RequestBody AccountCreateRequest request) {
		...
		if (!accountService.createAccount(request.getName(), request.getSurname(), request.getEmail(), request.getTc(),
				request.getType())) {
	...
	public ResponseEntity<Object> getDetailByAccountNumber(@PathVariable long accountNumber) {
		...
		if (!userService.isAuth(accountNumber)) {
			Response res = null;
			res.setMessage("Invalid Account Number");
			res.setSuccess(false);
		...
		return ResponseEntity.ok().lastModified(detail.getLastModified()).body(detail);
	}

	@PatchMapping("/{accountNumber}")
	public ResponseEntity<Object> deposit(@PathVariable long accountNumber,
	...
	public ResponseEntity<Object> transfer(@PathVariable long accountNumber,
		...
		try {
			res = accountService.moneyTransfer(accountNumber, request.getTransferredAccountNumber(),
					request.getAmount());
			if (!res.isSuccess()) {
				return ResponseEntity.badRequest().body(res);
			}
	...
	public ResponseEntity<Object> delete(@PathVariable long accountNumber) {
		...
		return ResponseEntity.ok().lastModified(acc.getLastModified()).body(res);

	}

	@CrossOrigin(origins = { "http://localhost:6162" })
	@GetMapping(path = "/logs/{accountNumber}")
	private ResponseEntity<Object> getLogs(@PathVariable(name = "accountNumber") long accountNumber) {
